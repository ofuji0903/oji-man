...
export default function Home() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!data.result || !data.result.text || !data.result.role) {
        console.warn("APIからのレスポンスが不正です", data.result);
        setLoading(false);
        return;
      }
      ...
      assistantMessage.text = assistantMessage.text.replace(/#信頼度:[+-]?\d+/, "").trim();
  ...
  const emotion = messages.findLast((m) => m.role === "assistant")?.emotion || "normal";
